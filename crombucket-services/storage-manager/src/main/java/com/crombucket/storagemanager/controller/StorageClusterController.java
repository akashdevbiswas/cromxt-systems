...
public class StorageClusterController {
    ...
    @GetMapping
    public Mono<ResponseEntity<ClustersListResponse>> getAllClusters(
            @RequestParam(required = false, defaultValue = "0") Integer pageNumber,
            @RequestParam(required = false, defaultValue = "20") Integer pageSize,
            @RequestParam(required = false, defaultValue = "NEWEST") SortingOrder order) {

        Mono<ClustersListResponse> clustersListResponseMono = storageClusterService.getAllStorageClusters(pageNumber, pageSize, order);
        return responseBuilder.buildResponseWithBody(clustersListResponseMono, HttpStatus.OK);
    }

    @GetMapping(value = "/{clusterId}")
    public Mono<ResponseEntity<ClusterResponse>> getClusterById(@PathVariable String clusterId) {
        Mono<ClusterResponse> clusterResponseMono = storageClusterService.getClusterById(clusterId);
        return responseBuilder.buildResponseWithBody(clusterResponseMono, HttpStatus.OK);
    }

    @DeleteMapping("/{clusterId}")
    public Mono<ResponseEntity<Void>> deleteCluster(@PathVariable String clusterId) {
